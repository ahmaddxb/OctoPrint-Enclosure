...
def is_active_low(val):
    if isinstance(val, dict):
        val = val.get('active_low', True)
...
def PinState_Boolean(pin, ActiveLow) :
    try:
        active_low = is_active_low(ActiveLow)
...
class EnclosurePlugin(octoprint.plugin.StartupPlugin, octoprint.plugin.ShutdownPlugin, octoprint.plugin.TemplatePlugin, octoprint.plugin.SettingsPlugin,
    ...
    def setup_gpio(self):
        # Setting board modes and warnings are no longer required under pure libgpiod use
        pass
    ...
    def configure_gpio(self):
        try:
            ...
            # Temperature sensors manage their own IO setups via dedicated libraries (Board/Adafruit_DHT/W1Term)
        except Exception as ex:
            self.log_error(ex)
    ...
    def write_gpio(self, gpio, value, queue_id=None):
        try:
            ...
            import gpiod
            set_success = False
            if gpio in active_gpiod_outputs:
                try:
                    active_gpiod_outputs[gpio].set_value(gpio, gpiod.line.Value.ACTIVE if val_bool else gpiod.line.Value.INACTIVE)
                    set_success = True
                except Exception as ex:
                    self._logger.warning("set_value failed on pin %s, re-requesting line: %s", gpio, ex)
                    try:
                        active_gpiod_outputs[gpio].release()
                    except:
                        pass
                    active_gpiod_outputs.pop(gpio, None)

            if not set_success and gpio not in active_gpiod_outputs:
                chip_path = '/dev/gpiochip4' if os.path.exists('/dev/gpiochip4') else '/dev/gpiochip0'
                self._logger.debug("Initializing new libgpiod output line %s on %s", gpio, chip_path)
    ...
    def get_update_information(self):
        return dict(enclosure=dict(displayName="Enclosure Plugin", displayVersion=self._plugin_version,
            ...
            type="github_release", user="ahmaddxb", repo="OctoPrint-Enclosure", current=self._plugin_version,
            # update method: pip
            pip="https://github.com/ahmaddxb/OctoPrint-Enclosure/archive/{target_version}.zip"))
